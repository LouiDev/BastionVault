...
internal sealed class TamperVault : IDisposable
{
    ...
    public async Task<IVaultSession> OpenTargetAsync(ulong seed = 5, Bastion.Core.Crypto.IKeyDerivation? kdf = null)
    {
        using Passphrase password = Passphrase.FromString(Password);
    ...
    /// <param name="kdf">Key-derivation seam; the real Argon2id when omitted.</param>
    public async Task<VaultException> ExpectOpenFailsAsync(byte[] bytes, string because, Bastion.Core.Crypto.IKeyDerivation? kdf = null)
    {
        Write(bytes);
