...
public sealed partial class ShellViewModel : ObservableObject, IDisposable
{
    ...
    public async Task<bool> RequestCloseAsync()
    {
        if (Operation.IsRunning)
        {
            await _dialogs.ShowInfoAsync(
                ...
                "Wait for it to finish, or cancel it, before closing Bastion Vault.").ConfigureAwait(true);
            return false;
        }
    ...
    public async Task NewVaultAsync()
    {
        ...
        if (_vaultLock is null)
        {
            ...
            await _dialogs.ShowInfoAsync(
                ...
                "Bastion Vault opened it in another window. Two processes writing one vault produce a conflict, not a merge.").ConfigureAwait(true);
            return;
        }
    ...
    private async Task<CurrentPasswordCheck> CheckCurrentPasswordAsync(IVaultSession session, Passphrase? current)
    {
        ...
        catch (NotImplementedException ex)
        {
            ...
            _log.Warn("This build of BastionVault.Core cannot verify a password; the current-password field was not checked.", ex);
            return CurrentPasswordCheck.Ok;
        }
    ...
    private async Task ReportAsync(string title, Exception ex)
    {
        ...
        string message = ex switch
        {
            ...
            NotImplementedException => "This part of BastionVault.Core is not implemented in this build.",
            _ => ex.Message,
        };
